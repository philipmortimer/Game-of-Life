...
type BrokerState struct {
	...
	StartGrid      [][]byte
	DistConnection *rpc.Client // Distributor connection
}
...
// reportCellsFlipped reports the list of flipped cells by looking at the newly computed grid
// and comparing it to the previous grid. This must be called before the world is updated
// but after the new state has been calculated.
func reportCellsFlipped() {
	rowsProcessed := 0
	var cellsFlipped []util.Cell
	for i := 0; i < len(s.Workers); i++ {
		for y := 0; y < len(s.Data.ServerChunks[i].GridChunk.Grid); y++ {
			for x := 0; x < len(s.Data.ServerChunks[i].GridChunk.Grid[y]); x++ {
				if s.Data.ServerChunks[i].GridChunk.Grid[y][x] != s.Data.ServerChunks[i].ServerResponse.Grid[y][x] {
					cellsFlipped = append(cellsFlipped, util.Cell{
						X: x,
						Y: y + rowsProcessed})
				}
			}
		}
		rowsProcessed += len(s.Data.ServerChunks[i].GridChunk.Grid)
	}
	if len(cellsFlipped) != 0 {
		s.DistConnection.Go(stubs.ReportCellsFlipped,
			stubs.ReportCellsFlippedRequest{CellsFlipped: cellsFlipped, TurnsCompleted: s.Turn + 1},
			new(stubs.ReportCellsFlippedResponse), nil)
	}
}

// updateWorld updates the grid values following server response
func updateWorld() {
	// Updates world and checks for cells flipped
	k.PauseSem.Wait()
	s.Lock.Lock()
	reportCellsFlipped()
...
func (b *OperationsBroker) ProcessGol(req stubs.ProcessGolRequest, response *stubs.ProcessGolResponse) (err error) {
	s.Lock.Lock() // Reports initial game state to sdl
	s.DistConnection.Go(stubs.ReportCellsFlipped,
		stubs.ReportCellsFlippedRequest{TurnsCompleted: s.Turn, CellsFlipped: getAliveCells()},
		new(stubs.ReportCellsFlippedResponse), nil)
	s.Lock.Unlock()
	...
	response.AliveCells = getAliveCells()
	response.TurnsCompleted = s.Turn
	s.DistConnection.Close()
	s.DistConnection = nil
	s.Lock.Unlock()
	return nil
}
...
// connectToDistributor establishes rpc connection with the distributor
func connectToDistributor() {
	client, err := rpc.Dial("tcp", stubs.DistributorAddress)
	s.DistConnection = client
	check(err)
}
...
func main() {
	...
	serverAdds := flag.String("serverAddressesList",
		"127.0.0.1:8038,127.0.0.1:8039,127.0.0.1:8040",
