...
func distributor(p Params, c distributorChannels, keyPresses <-chan rune) {
	...
	initSyncData(len(algoData.ThreadChunks))
	...
	for turn < p.Turns && !quit {
		...
		default:
			if !paused { // By default, will evolve board state by one turn and update grids
				evolveGrid(&algoData, &turn, c.events)
			}
		}
	}
...
func evolveGrid(grids *GameOfLifeData, turn *int, events chan<- Event) {
	...
	for i := range grids.ThreadChunks {
		go nextState(grids.CurrentGrid, grids.NewGrid, grids.ThreadChunks[i], *turn, events,
...
func nextState(world, newWorld [][]byte, chunk WorldChunk, turn int, events chan<- Event, gridHeight, gridWidth int) {
