...
type ioChannels struct {
	...
	idle    chan<- bool
	// the input comes from reading the pgm file, the output is a receive only channel,
	//sent to the distributer i guess ?
...
func (io *ioState) readPgmImage() {
	...
	image := []byte(fields[4])
	// this is reading the image byte and sending it down to the distributor
	for _, b := range image {
		io.channels.input <- b
	}
