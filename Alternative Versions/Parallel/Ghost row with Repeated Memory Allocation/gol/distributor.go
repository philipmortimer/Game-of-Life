...
// PartialChunk stores a section of GoL board but doesn't store ghost rows
type PartialChunk struct {
	Chunk      [][]byte
	ChunkIndex int // ChunkIndex indicates which element from WorldChunk array this struct maps to
}
...
func distributor(p Params, c distributorChannels, keyPresses <-chan rune) {
	world := getBoard(p, c)                          // Reads in GoL life grid
	boardChan := make(chan PartialChunk, len(world)) // Channel process sends grid state to
	...
	for turn < p.Turns && !quit {
		...
		case <-ticker.C:
			if !paused { // Sends number of alive cells
				c.events <- AliveCellsCount{CellsCount: len(aliveCells(world)), CompletedTurns: turn}
			}
		default:
			if !paused { // By default, will evolve board state by one turn
				world = evolveGrid(world, boardChan, turn, c.events)
				turn++
	...
	gameFinished(c, turn, world)  // Ensures GoL finish occurs correctly2
}

// evolveGrid evolves GoL grid by 1 turn and returns new grid
func evolveGrid(world []WorldChunk, boardChan chan PartialChunk, turn int, events chan<- Event) []WorldChunk {
	newWorld := make([]WorldChunk, len(world)) // Used to store updated grid
	// Starts each of worker threads
	for i := range world {
		go nextState(world[i], i, boardChan, events, turn)
	}
	// Receives processed chunks
	for chunksReceived := 0; chunksReceived < len(world); chunksReceived++ {
		part := <-boardChan
		newWorld[part.ChunkIndex] = WorldChunk{Chunk: part.Chunk, AboveRow: world[part.ChunkIndex].AboveRow,
			BelowRow: world[part.ChunkIndex].BelowRow, StartY: world[part.ChunkIndex].StartY}
	}
	// Updates above and below rows
	for chunkIndex := range newWorld {
		// Updates below row of previous chunk
		prevChunkIndex := chunkIndex - 1
		if prevChunkIndex < 0 {
			prevChunkIndex = len(newWorld) - 1
		}
		copy(newWorld[prevChunkIndex].BelowRow, newWorld[chunkIndex].Chunk[0])
		// Updates above row of next chunk
		nextChunkIndex := (chunkIndex + 1) % len(newWorld)
		copy(newWorld[nextChunkIndex].AboveRow, newWorld[chunkIndex].Chunk[len(newWorld[chunkIndex].Chunk)-1])
	}
	return newWorld
}
...
func initEmptyChunkStruct(p Params) []WorldChunk {
	...
	for chunkIndex := 0; chunkIndex < len(chunks); chunkIndex++ {
		...
		chunks[chunkIndex] = WorldChunk{makeTwoDMatrix(height, p.ImageWidth),
			make([]byte, p.ImageWidth), make([]byte, p.ImageWidth), totalRowsAssigned}
...
func getBoard(p Params, c distributorChannels) []WorldChunk {
	...
	for chunkIndex := 0; chunkIndex < len(chunks); chunkIndex++ {
		...
		// Sets below ghost row of previous chunk
		prevChunkIndex := chunkIndex - 1
		if prevChunkIndex < 0 {
			prevChunkIndex = len(chunks) - 1
		}
		copy(chunks[prevChunkIndex].BelowRow, chunks[chunkIndex].Chunk[0])
		// Sets above ghost row of next chunk
		nextChunkIndex := (chunkIndex + 1) % len(chunks)
		copy(chunks[nextChunkIndex].AboveRow, chunks[chunkIndex].Chunk[len(chunks[chunkIndex].Chunk)-1])
	}
	return chunks
}
...
// nextState Runs a single iteration of GoL algorithm on a chunk of grid and returns the new chunk to channel.
func nextState(world WorldChunk, chunkIndex int, boardChan chan<- PartialChunk, events chan<- Event, turn int) {
	// Creates a new chunk struct to store new world.
	newWorld := PartialChunk{Chunk: makeTwoDMatrix(len(world.Chunk), len(world.Chunk[0])), ChunkIndex: chunkIndex}
	// Loops through cell's and applies GoL rules to calculate new state
	for y, row := range world.Chunk {
		for x, cellCont := range row {
			neighboursAlive := aliveNeighbours(y, x, world)
			// Updates new world cell in chunk based on calculation
			if cellCont == Alive && neighboursAlive < 2 {
				newWorld.Chunk[y][x] = Dead
			} else if cellCont == Alive && neighboursAlive > 3 {
				newWorld.Chunk[y][x] = Dead
			} else if cellCont == Dead && neighboursAlive == 3 {
				newWorld.Chunk[y][x] = Alive
			} else {
				newWorld.Chunk[y][x] = cellCont
			}
			// Sends cell flipped notification
			if newWorld.Chunk[y][x] != cellCont {
				events <- CellFlipped{Cell: util.Cell{X: x, Y: y + world.StartY}, CompletedTurns: turn}
			}
		}
	}
	boardChan <- newWorld // Sends data to channel
}

// Calculates all alive cells in GoL world
	...
	for chunkIndex := range world {
		for y, row := range world[chunkIndex].Chunk {
			for x, cell := range row {
				if cell == Alive {
					alive = append(alive, util.Cell{X: x, Y: y + world[chunkIndex].StartY})
				}
			}
		}
	}
